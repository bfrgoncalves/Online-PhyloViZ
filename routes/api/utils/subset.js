	...
	if (req.isAuthenticated()){
		...
		fs.writeFile("uploads/"+profileName, req.body.profileData, function(err) {
		    ...
		    fs.writeFile("uploads/"+metadataName, req.body.auxData, function(err) {
			    ...
			    if(req.body.missings == 'true') var commandstring = 'cd remote_upload/ & python remoteUpload.py -t '+cookie_string+' -cd '+config.final_root+' -root '+config.final_root+' -mc ' + req.body.missingschar + ' -sdt profile -sd uploads/'+profileName+' -d '+req.body.name+' -dn '+req.body.description+' -m uploads/'+metadataName;
			    else var commandstring = 'cd remote_upload/ & python remoteUpload.py -t '+cookie_string+' -cd '+config.final_root+' -root '+config.final_root+' -sdt profile -sd uploads/'+profileName+' -d '+req.body.name+' -dn '+req.body.description+' -m uploads/'+metadataName;
