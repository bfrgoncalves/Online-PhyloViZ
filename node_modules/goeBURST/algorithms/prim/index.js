...
function goeBURST_prim(profiles, identifiers, missings, analysis_method, threshold, callback) {
  ...
  filter_profile(profiles, identifiers, missings, analysis_method, threshold, function(indexToRemove, profiles){
    ...
    while (! pqueue.empty()) {
      ...
      color[u] = 2;
      if (u != 0){
        var comp = comparator(profiles[pi[u][0]], profiles[pi[u][1]]);

        if (missings[0] == true && analysis_method == 'pairwise' || analysis_method == 'pair+tie') {
          var missingsToUse = comp[1];
        }
        else {
          var missingsToUse = undefined;
        }
        tree.push({source: identifiers[pi[u][0]], target: identifiers[pi[u][1]], value: comp[0], missings: missingsToUse});
      }
    ...
    function edgecmp(e, f) {
      ...
      for (var l = 0; l < n; l++) {
        var maxe = Math.max(lvs[e[0]][l], lvs[e[1]][l]);
        var maxf = Math.max(lvs[f[0]][l], lvs[f[1]][l]);

        if (maxe != maxf)
          return maxf - maxe;

        var mine = Math.min(lvs[e[0]][l], lvs[e[1]][l]);
        var minf = Math.min(lvs[f[0]][l], lvs[f[1]][l]);
      ...
      var maxe = Math.max(e[0], e[1]);
      var maxf = Math.max(f[0], f[1]);
