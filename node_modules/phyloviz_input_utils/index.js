...
function addToFilterTable(graph, userID, datasetID, callback){
	...
	var KeysToSave = ['mergedNodes', 'sameProfileHas', 'links', 'sameNodeHas', 'subsetProfiles', 'usedLoci', 'nodes', 'goeburstprofilesize', 'missingsInfo', 'distanceMatrix'];
    ...
    client.connect(function(err) {
    	if(err) {
  		...
  		function runQuery(index){
  			...
  			if(keyToUse == 'links') query = "UPDATE datasets.links SET data = $1 WHERE dataset_id ='"+graph.datasetID+"';";
  			else query = "INSERT INTO datasets.filters (user_id, dataset_id, data, data_timestamp) VALUES ('"+userID+"', '"+datasetID+"', $1, NOW());";
	  ...
	  if(err) {
		...
		client.query(query, function(err, result) {
		    ...
		    for(row in result.rows){
		    	...
		    	if(keyT == 'nodes' || keyT == 'subsetProfiles'){
...
function FlushFunction(graphInput, res, callback){
	...
	//res.setMaxListeners(0); //set listerners to infinity
		...
		function runFlush(index){
			...
			else if(arrayOfKeys[index] == 'mergedNodes' || arrayOfKeys[index] == 'sameNodeHas' || arrayOfKeys[index] == 'usedLoci' || arrayOfKeys[index] == 'indexesToRemove'){
					...
					if(arrayOfKeys[index] == 'indexesToRemove'){
		              ...
		              //console.log()
						...
						var toSend = '{"' + arrayOfKeys[index] + '":' + JSON.stringify(nodeSlice) + '}';
	  					...
	  					populateArray(toSend, profileLength, 0, function(){
