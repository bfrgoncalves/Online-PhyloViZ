...
function FlushFunction(graphInput, res, callback){
	...
	//res.setMaxListeners(0); //set listerners to infinity
		...
		function runFlush(index){
			...
			*/
			else if(arrayOfKeys[index] == 'nodes' || arrayOfKeys[index] == 'subsetProfiles' || arrayOfKeys[index] == 'distanceMatrix' || arrayOfKeys[index] == 'usedLoci' ||arrayOfKeys[index] == 'indexesToRemove' || arrayOfKeys[index] == 'mergedNodes' || arrayOfKeys[index] == 'sameNodeHas' || arrayOfKeys[index] == 'sameProfileHas'){
				index += 1;
				if(index < numKeys) runFlush(index);
				else{
			      res.end();
