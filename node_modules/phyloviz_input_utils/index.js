		...
		client.connect(function(err) {
		  ...
		  if(isPublic == true){
		  	query = "SELECT data AS profiles, schemeGenes FROM datasets.profiles WHERE dataset_id='"+datasetID+"';"+
		  			...
		  			"SELECT key, data_timestamp FROM datasets.datasets WHERE dataset_id='"+datasetID+"' LIMIT 1;" +
		  	...
		  	query = "SELECT data AS profiles, schemeGenes FROM datasets.profiles WHERE (dataset_id='"+datasetID+"' AND user_id='"+userID+"') OR (dataset_id='"+datasetID+"' AND is_public='t');" +
		  			...
		  			"SELECT key, data_timestamp, data_type FROM datasets.datasets WHERE (dataset_id='"+datasetID+"' AND user_id='"+userID+"') OR (dataset_id='"+datasetID+"' AND is_public='t') LIMIT 1;" +
		    ...
		    client.query(query, function(err, result) {
			    ...
			    for(i in result.rows){
			    	for(x in result.rows[i]){
			    		if( x == 'profiles') {
			    			//console.log(result.rows[i][x]['profiles']);
			    			...
			    			if (result.rows[i][x].hasOwnProperty('analysis_method')) dataset.analysis_method = result.rows[i][x]['analysis_method'];
			    			if (result.rows[i][x].hasOwnProperty('missing_threshold')) dataset.missing_threshold = result.rows[i][x]['missing_threshold'];
			    			if (result.rows[i][x].hasOwnProperty('goeburst_timer')) dataset.goeburst_timer = result.rows[i][x]['goeburst_timer'];
			    			if (result.rows[i][x].hasOwnProperty('parent_id')) dataset.parent_id = result.rows[i][x]['parent_id'];
			    		}
			    		else if( x  == 'links'){
			    		...
			    		else if( x == 'schemegenes' || x == 'metadata' || x == 'key' || x == 'data_timestamp' || x == 'data_type'){
			    			//console.log(result.rows[i][x]);
			    			try{
			    				dataset[x] = result.rows[i][x].toString().replace(/&39/g, "'").split(',');
			    			}
			    			catch(err){
			    				dataset[x] = [];
			    			}
...
function addToFilterTable(graph, userID, datasetID, overrideArray, callback){
	...
	else var KeysToSave = ['mergedNodes', 'sameProfileHas', 'sameNodeHas', 'subsetProfiles', 'usedLoci', 'nodes', 'goeburstprofilesize', 'missingsInfo', 'distanceMatrix', 'goeBURSTschemeGenesExport', 'goeburst_timer', 'parent_id', 'analysis_method', 'missing_threshold', 'maxDistanceValue'];
	...
	console.log("NODES SPLICE", NodesToSplice.length);
	var SubsetNodesToSplice = graph['subsetProfiles'].slice(0, graph['subsetProfiles'].length);
	
	//case of legacy datasets
	if(!graph.hasOwnProperty('distanceMatrix')) var MatrixToSplice = [];
	else var MatrixToSplice = graph['distanceMatrix'].slice(0, graph['distanceMatrix'].length);
    	...
    	if(err) {
  		...
  		function runQuery(index){
  			...
  			client.query(query, [dataToUse], function(err, result) {
	            ...
	            console.log("Nodes TO SPLCIE LENGTH", NodesToSplice.length);
	...
	client.connect(function(err) {
	  if(err) {
		...
		query = "SELECT data FROM datasets.filters WHERE dataset_id = '"+dataset_id+"';" +
				"SELECT key, data_timestamp FROM datasets.datasets WHERE dataset_id='"+dataset_id+"' LIMIT 1;";
		...
		client.query(query, function(err, result) {
		    ...
		    for(row in result.rows){
		    	...
		    	//console.log(resultObject);
		    	var keyT = Object.keys(resultObject)[0];
		    	console.log(keyT, Object.keys(resultObject));
		    	//console.log(keyT);
		    	if(keyT == 'nodes' || keyT == 'subsetProfiles' || keyT == 'distanceMatrix'){
		    		if(!graph.hasOwnProperty(keyT)) graph[keyT] = [];
		    		console.log(keyT, resultObject[keyT].length);
		    		graph[keyT] = graph[keyT].concat(resultObject[keyT]);

		    	}
		    	else if(keyT == 'key'){
		    		graph[keyT] = [resultObject[keyT]];
		    		if(resultObject.hasOwnProperty('data_timestamp')) graph['data_timestamp'] = [resultObject['data_timestamp']];
		    	}
...
function FlushFunction(graphInput, res, callback){
	...
	//res.setMaxListeners(0); //set listerners to infinity
		...
		function runFlush(index){
			...
			if(arrayOfKeys[index] == 'nodes' || arrayOfKeys[index] == 'subsetProfiles' || arrayOfKeys[index] == 'links' || arrayOfKeys[index] == 'distanceMatrix'){
				if(graphInput[arrayOfKeys[index]].length != 0){
					batches += 1;

					var bsize = graphInput[arrayOfKeys[index]].length;

					var nodeSlice = graphInput[arrayOfKeys[index]].splice(0, 1); //config.batchSize
					
					if (arrayOfKeys[index] == 'distanceMatrix' && nodeSlice[0].length > 3000){
						index += 1;
						firstTimeinIndex = true;
						if(index < numKeys) return runFlush(index);
						else{
					      res.end();
					      return;
					...
					else if(arrayOfKeys[index] == 'distanceMatrix' && nodeSlice[0].length <= 3000) var tout = 50;
					else var tout = 5;
					//console.log('BATCH ', batches);
					
					if(firstTimeinIndex) var toSend = '{"' + arrayOfKeys[index] + '":' + JSON.stringify(nodeSlice) + ', "size":'+String(bsize)+'}';
					else var toSend = '{"' + arrayOfKeys[index] + '":' + JSON.stringify(nodeSlice) + '}';

					firstTimeinIndex = false;
					//if(arrayOfKeys[index] == 'distanceMatrix') var tout = 50;
			...
			//arrayOfKeys[index] == 'usedLoci'
				...
				if(graphInput[arrayOfKeys[index]] == undefined){
					...
					if(firstTimeinIndex) var toSend = '{"' + arrayOfKeys[index] + '":' + JSON.stringify(nodeSlice) + ', "size":'+JSON.stringify(Object.keys(graphInput[arrayOfKeys[index]]).length)+'}';
					else var toSend = '{"' + arrayOfKeys[index] + '":' + JSON.stringify(nodeSlice) + '}';
			...
			else if(arrayOfKeys[index] == 'usedLoci' ||arrayOfKeys[index] == 'indexesToRemove' || arrayOfKeys[index] == 'sameProfileHas'){
				index += 1;
				firstTimeinIndex = true;
				if(index < numKeys) runFlush(index);
				else{
				...
				if (arrayOfKeys[index] == 'schemeGenes') var tout = 100;
				else var tout = 20;
