...
function FlushFunction(graphInput, res, callback){
	...
	//res.setMaxListeners(0); //set listerners to infinity
		...
		function populateArray(dataString, profileLength, nodeLength, timeO, callback) {

		    send(dataString, profileLength, nodeLength, timeO, function(){
		    	callback();
		    });
		}

		function send(dataString, profileLength, nodeLength, timeO, callback) {
		...
		function runFlush(index){
			...
			if(arrayOfKeys[index] == 'nodes' || arrayOfKeys[index] == 'subsetProfiles' || arrayOfKeys[index] == 'links' || arrayOfKeys[index] == 'distanceMatrix'){
				if(graphInput[arrayOfKeys[index]].length != 0){
					...
					if (arrayOfKeys[index] == 'distanceMatrix' && nodeSlice.length > 3000) var tout = 100;
					else if (arrayOfKeys[index] == 'distanceMatrix' && nodeSlice.length > 1000) var tout = 50;
					else if (arrayOfKeys[index] == 'distanceMatrix' && nodeSlice.length > 30) var tout = 10;
					else var tout = 1;
  					populateArray(toSend, profileLength, nodeLength, tout, function(){
			...
			//arrayOfKeys[index] == 'usedLoci'
				...
				if(graphInput[arrayOfKeys[index]] == undefined){
					...
					var tout = 10;
  					...
  					populateArray(toSend, profileLength, 0, tout, function(){
			...
			/*
			else if(arrayOfKeys[index] == 'distanceMatrix' || arrayOfKeys[index] == 'usedLoci' ||arrayOfKeys[index] == 'indexesToRemove'){
				index += 1;
				runFlush(index);
			}
