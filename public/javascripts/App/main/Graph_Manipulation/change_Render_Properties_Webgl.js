...
function NLVcollapse(graphObject, value) {
    ...
    if (value < prevValue && nodes_at_distance[prevValue] != undefined && links_at_distance[prevValue] != undefined){
        ...
        for(k in nodes_at_distance[prevValue]){
            ...
            graphGL.addLink(graph.sameNodeHas[nodes_at_distance[prevValue][k][0].id], graph.sameNodeHas[nodes_at_distance[prevValue][k][1].id], graphObject.links_id_to_data[nodes_at_distance[prevValue][k][0].id + "👉 " + nodes_at_distance[prevValue][k][1].id]);
        }
        /*countAddedLinks = 0;
