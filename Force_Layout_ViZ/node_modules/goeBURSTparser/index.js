...
var parseLinks = function(data, nodesIndex){
	...
	data.toString().split('\n').forEach(function(d){
		...
		Links.push('{ "source" : ' + String(nodesIndex[source]) + ' ,"target" : ' + String(nodesIndex[target]) + ', "value" : ' + weight + '}');
	});
...
var createJSON = function(ToJSON, Metadata){
	...
	csvMetadata.on('end',function(){
		...
		csvNodes.on('end',function(){
			...
			fs.readFile(links, 'utf8', function(err, data){
		  		...
		  		ToJSON.push(Links);
		  		toWrite = createJSON(ToJSON, Metadata);
